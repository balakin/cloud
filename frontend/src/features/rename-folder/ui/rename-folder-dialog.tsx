...
export type RenameFolderDialogProps = Omit<DialogProps, 'onClose'> & {
  id: string;
  name: string;
  onClose?: () => void;
};

export const RenameFolderDialog: FC<RenameFolderDialogProps> = ({ id, name, onClose, ...props }) => {
  ...
  const formik = useFormik<ChangeFolderDto>({
    initialValues: {
      name,
    },
        ...
        <Form>
          <DialogContent>
            <FormStack>
              ...
              <FormTextField name={nameof<ChangeFolderDto>('name')} label="Name" />
            </FormStack>
          </DialogContent>
          <DialogActions>
